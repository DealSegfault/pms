...
class OrderManager:
    ...
    async def place_market_order(
        ...
        **kwargs,
        ...
        order = OrderState(
            ...
            origin=origin,
            parent_id=parent_id,
            order_role=normalize_order_role(order_role),
    ...
    async def place_batch_limit_orders(
        ...
        for p in order_params:
            ...
            order = OrderState(
                ...
                order_role=normalize_order_role(p.get("order_role", "UNKNOWN")),
                strategy_session_id=p.get("strategy_session_id"),
                parent_strategy_session_id=p.get("parent_strategy_session_id"),
                root_strategy_session_id=p.get("root_strategy_session_id"),
                replaces_client_order_id=p.get("replaces_client_order_id"),
    ...
    async def replace_order(
        ...
        result = await self.place_limit_order(
            ...
            order_role="REPRICE",
            strategy_session_id=getattr(old, "strategy_session_id", None),
            parent_strategy_session_id=getattr(old, "parent_strategy_session_id", None),
            root_strategy_session_id=getattr(old, "root_strategy_session_id", None),
            replaces_client_order_id=old.client_order_id,
            on_fill=old.on_fill,
    ...
    async def _create_bot_order_from_feed(self, data: dict) -> Optional[OrderState]:
        ...
        order = OrderState(
            ...
            order_role=normalize_order_role(data.get("order_role", "UNKNOWN")),
            strategy_session_id=data.get("strategy_session_id") or None,
            parent_strategy_session_id=data.get("parent_strategy_session_id") or None,
            root_strategy_session_id=data.get("root_strategy_session_id") or None,
            replaces_client_order_id=data.get("replaces_client_order_id") or None,
